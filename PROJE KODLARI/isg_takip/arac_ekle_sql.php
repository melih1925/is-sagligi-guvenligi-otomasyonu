...
if ($kul_id === 0) {
    $_SESSION['errors'] = ["Kullanıcı oturumu bulunamadı."];
    header("Location: arac_ekle.php");
    exit;
}
...
// Araç bilgilerini al ve temizle
...
if (empty($plaka_no)) {
    ...
    // Plaka no benzersizlik kontrolü
...
if (!empty($muayene_tarihi) || !empty($muayene_sonucu)) {
    if (empty($muayene_tarihi)) {
        ...
        if (!$muayene_date || $muayene_date > $currentDateTime) {
            $errors[] = "Muayene tarihi geçerli değil veya gelecekte olamaz. $currentDateTime";
        }
    }
}
...
try {
    ...
    $sorgu = $db->prepare("INSERT INTO arac_bilgi (plaka_no, arac_tipi_id, marka_id, model_id, uretim_yili, firma_id, arac_durum_id, kullanici_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    $sorgu->execute([$plaka_no, $arac_tipi_id, $marka_id, $model_id, $uretim_yili, $firma_id, $arac_durum_id, $kul_id]);
    $arac_id = $db->lastInsertId();

    // Muayene bilgileri varsa kaydet
    if (!empty($muayene_tarihi) && !empty($muayene_sonucu)) {
        ...
        $sorgu = $db->prepare("INSERT INTO arac_muayene (arac_id, muayene_tarihi, muayeneden_gecti_mi, muayene_gecerlilik_tarihi, kullanici_id) VALUES (?, ?, ?, ?, ?)");
        $sorgu->execute([$arac_id, $muayene_tarihi, $muayeneden_gecti_mi, $gecerlilik_tarihi, $kul_id]);
    }

    // Operatör atama varsa kaydet
    if (!empty($tc_kimlik) && !empty($atama_tarihi)) {
        $sorgu = $db->prepare("
    ...
    $_SESSION['success'] = "Araç ve ilgili bilgiler başarıyla kaydedildi.";
