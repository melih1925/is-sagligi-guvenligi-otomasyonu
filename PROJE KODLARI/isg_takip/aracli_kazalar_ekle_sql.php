...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['kaza_ekle'])) {
    ...
    // Kaza kaydını ekle
    $insert = $db->prepare("
        INSERT INTO aracli_kazalar 
        (arac_id, tc_kimlik, is_kazasi_tip_id, yaralanma_durumu_id, yaralanma_tip_id, kaza_aciklamasi, is_kazasi_tarihi, kullanici_id) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ");
    $success = $insert->execute([$arac_id, $tc_kimlik, $is_kazasi_tip_id, $yaralanma_durumu_id, $yaralanma_tip_id, $kaza_aciklamasi, $is_kazasi_tarihi, $kul_id]);
